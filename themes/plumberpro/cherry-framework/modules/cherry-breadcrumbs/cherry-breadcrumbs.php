	...
	class Cherry_Breadcrumbs {
		...
		public function default_labels() {
			...
			if ( $use_custom_front_title ) {
				$default = __( 'Home', 'plumberpro' );
				$page_on_front_title = $default;
				$page_on_front_title = $this->prepare_label( $page_on_front_title, $default );
			}

			$labels = array(
				'browse'              => __( 'Browse:',                             'plumberpro' ),
				'home'                => $page_on_front_title,
				'error_404'           => __( '404 Not Found',                       'plumberpro' ),
				'archives'            => __( 'Archives',                            'plumberpro' ),
				/* Translators: %s is the search query. The HTML entities are opening and closing curly quotes. */
				'search'              => __( 'Search results for &#8220;%s&#8221;', 'plumberpro' ),
				/* Translators: %s is the page number. */
				'paged'               => __( 'Page %s',                             'plumberpro' ),
				/* Translators: Minute archive title. %s is the minute time format. */
				'archive_minute'      => __( 'Minute %s',                           'plumberpro' ),
				/* Translators: Weekly archive title. %s is the week date format. */
				'archive_week'        => __( 'Week %s',                             'plumberpro' ),
		...
		public function add_minute_hour_archive_items() {
			...
			$label = sprintf(
				...
				get_the_time( _x( 'g:i a', 'minute and hour archives time format', 'plumberpro' ) )
			);
			$this->_add_item( 'target_format', $label );
		...
		public function add_minute_archive_items() {
			...
			$label = sprintf(
				...
				get_the_time( _x( 'i', 'minute archives time format', 'plumberpro' ) )
			);
		...
		public function add_hour_archive_items() {
			...
			$label = sprintf(
				...
				get_the_time( _x( 'g a', 'hour archives time format', 'plumberpro' ) )
			);
			$this->_add_item( 'target_format', $label );
		...
		public function add_day_archive_items() {
			...
			$year = sprintf(
				...
				get_the_time( _x( 'Y', 'yearly archives date format',  'plumberpro' ) )
			);

			$month = sprintf(
				$this->args['labels']['archive_month'],
				get_the_time( _x( 'F', 'monthly archives date format', 'plumberpro' ) )
			);

			$day = sprintf(
				$this->args['labels']['archive_day'],
				get_the_time( _x( 'j', 'daily archives date format',   'plumberpro' ) )
			);
		...
		public function add_week_archive_items() {
			...
			$week = sprintf(
				...
				get_the_time( _x( 'W', 'weekly archives date format', 'plumberpro' ) )
			);
